...
const adapterName = "NanoBananaAdapter"

// Adapter implements ModelAdapter for Google Nano Banana / Imagen image editing (OS).
type Adapter struct {
	cfg    Config
	client Client
}

// NewAdapter creates the OS NanoBanana adapter.
...
// Region is always OS for NanoBanana.
...
// Invoke translates input, calls Nano Banana via GCP proxy, and returns output object keys.
