...
func TestNormalizeVendorError(t *testing.T) {
	cases := []struct {
		...
		{"RESOURCE_EXHAUSTED", http.StatusTooManyRequests, adapter.ErrCodeRateLimited, true},
		{"UNAVAILABLE", http.StatusInternalServerError, adapter.ErrCodeUpstream, true},
		{"DEADLINE_EXCEEDED", http.StatusOK, adapter.ErrCodeTransient, true},
		{"FACE_NOT_DETECTED", http.StatusOK, adapter.ErrCodeFaceNotFound, false},
		{"SAFETY_FILTER", http.StatusOK, adapter.ErrCodeContentPolicy, false},
		{"INVALID_ARGUMENT", http.StatusBadRequest, adapter.ErrCodeInvalidInput, false},
	}
